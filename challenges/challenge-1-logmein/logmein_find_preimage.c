...
int main() {
  ...
  // Make the symbolic input not exceed the buffer to not trigger any vulnerability
  klee_assume(sizeof(input) <= INPUT_SIZE);
  // Ensure null-terminationklee-r
  klee_assume(input[INPUT_SIZE - 1] == '\0');
  ...
  klee_assert(my_memcmp(buf, (void *)password_hash, BLOCK_SIZE) == 0);
